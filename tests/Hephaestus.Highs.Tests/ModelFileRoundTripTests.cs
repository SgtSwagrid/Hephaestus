...
public sealed class ModelFileRoundTripTests {
    private static readonly ContinuousVariable A = Variable.Continuous("departure A");
    private static readonly ContinuousVariable B = Variable.Continuous("departure B");
    ...
    public static TheoryData<string, IProblem> Problems => new() {
        { "headway", Problem.Minimise(A + 2 * B + 7, subjectTo: A.Between(0, 3600) & B.Between(30, 3600) & (((A + 120 <= B) | (B + 120 <= A)).WithName("headway")) & (A >= 10)) },
    ...
    public void HighsReadsBackWhatWasWritten(string name, IProblem problem) {
