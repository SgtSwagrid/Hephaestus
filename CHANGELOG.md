...
- A projected expression is readable, writable or both: `IReadableExpression<out TValue>` carries a decoder, `IWritableExpression<in TValue>` an encoder, and `ILinearlyEncodable<TValue>` is both by way of its projection. `solution.Value` now takes anything readable.
