...
- Expressions have a root that names what can be done with them: `IReadableExpression<out TValue>` (a solution gives a value), `IWritableExpression<in TValue>` (a plain value can stand for it) and `IExpression<TValue>` for both. `ILinearExpression`, `IBooleanExpression` and `ILinearlyEncodable<TValue>` are all `IExpression<TValue>`.
...
- Projections can be re-viewed: `Biselect` gives a projection of another type, `Select` gives a decoder alone (a reading no constraint can mention), and `Preselect` gives an encoder alone.
