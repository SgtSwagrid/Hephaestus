...
- `Vector<T>`, a fixed number of expressions combined entry by entry through `Zip` and `Select`: `u.Zip(v).Select((a, b) => a + b)`. Vectors of quantities add, subtract, scale, `Sum`, take a `Dot` product with plain weights, and compare with vectors, plain arrays and plain values in every entry (`u + v < [4, 5, 6]`). A vector of anything readable, points and constraints included, is read back as `u.Select(solution.Value)`.
